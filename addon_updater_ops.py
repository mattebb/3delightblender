...
updater.addon = "renderman_for_blender"
...
class addon_updater_install_popup(bpy.types.Operator):
	...
	bl_label = "Update Renderman for Blender addon".format(x=updater.addon)
...
class addon_updater_check_now(bpy.types.Operator):
	...
	bl_description = "Check now for an update to the Renderman for Blender addon".format(
														x=updater.addon)
...
class addon_updater_update_now(bpy.types.Operator):
	...
	bl_description = "Update to the latest version of the Renderman for Blender addon".format(
														x=updater.addon)
...
class addon_updater_update_target(bpy.types.Operator):
	...
	bl_description = "Install a targeted version of the Renderman for Blender addon".format(
														x=updater.addon)
...
def register(bl_info):
	...
	# updater.set_check_interval(enable=False,months=0,days=1,hours=0,minutes=0)
